variable "nms_private_subnets" {
    description = "Private Subnet ID for RDS DB"
    type = list(any)
}

variable "nms_private_subnet_cidr" {
    description = "Private subnet CIDR Block for RDS DB"
    type = list(any)
...
variable "db_az" {
  description = "DB Availabiltiy Zones"
  type = string
  validation {
    condition = can(regex("^[a-zA-Z0-9\\-]+$", var.db_az))
    error_message = "DB AZ must not empty"
